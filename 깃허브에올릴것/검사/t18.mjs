...
const rs = await pg.evaluate(()=>{
  ...
  for(let i=0; i<bag.max && W.__XP.pts>0; i++) W.__takeStat(1);
  o.주머니 = W.__XP.st[1];
  o.가득 = dig();
  return o;
});
ok('★ 큰 주머니를 8까지 찍는다', rs.주머니상한 === 8, rs.주머니상한);
ok('★ 만렙이 35 다', rs.만렙 === 35, rs.만렙);
ok('★ 만렙이어도 스텟을 다 못 찍는다 (골라야 한다)',
   rs.스텟합 > rs.만렙, '상한 합계 '+rs.스텟합+' > 점수 '+rs.만렙);
ok('★ 주머니를 찍을수록 캐는 양이 는다', rs.가득 > rs.맨손,
   '맨손 '+rs.맨손+' → 주머니'+rs.주머니+'점 '+rs.가득);
