/* 17차 검사 — 격자 오각형 맵 · 초반 난이도 · 자원/스텟 · 좀비 분산 · 레벨 35
...
ok('★ 다섯 입구 어귀가 다 직선이다 (통로 사람 끝 산이 같은 자리에서 시작)',
   mp.어귀표본 >= 4 && mp.어귀들쭉 === 0,
...
ok('★ 초반 좀비가 후반에 비해 덜 뒤처진다 (5일차가 15일차의 20%는 된다)',
...
const rs = await pg.evaluate(()=>{
  ...
  o.만렙 = W.__LV_MAX;
  /* 한 번 캘 때 실제로 들어오는 사람 — 스텟 0점일 때와 만점일 때 */
  ...
  /* ★ 창고를 0 으로 두면 안 된다 — 앞 검사에서 벽을 예순 채 넘게 지어 myPC 의 '쓴 사람'이
...
const sp = await pg.evaluate(()=>{
  ...
  o.남은좀비 = G.wolves.length;
  G.wolves.length=0; W.__clear();
  return o;
});
ok('벽 한 줄을 세우고 좀비 18마리를 붙였다', sp.벽수 >= 13 && sp.총피해 > 0,
   sp.벽수+'칸 · 총피해 '+sp.총피해);
...
/* ★ 처음엔 '0.01% 미만' 으로 걸었는데, 여덟 번 재 보니 관측값이 0~5 라
   ...
   ★ 그래서 '얼마나 자주' 대신 '한때 몇 쌍이나' 를 본다. 밀어내기는 겹친 만큼을
     몇 프레임에 걸쳐 푸는 물렁한 장치라, 뒤에서 밀린 좀비 한 쌍이 한순간 깊이 눌리는 건
     물리지 버그가 아니다. 눈에 보이는 '무더기' 는 여러 쌍이 동시에 포개진 것이다. */
ok('★ 좀비 무더기가 생기지 않는다 (한때 포개진 쌍이 둘 이하)',
