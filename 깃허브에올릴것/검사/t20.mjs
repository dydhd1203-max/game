...
const pos = await pg.evaluate(()=>{
  ...
  /* 성벽(입구) 쪽으로 나가 있으면 좀비 길을 막는다 — 수정보다 가까이 */
...
ok('★ 농장이 마당 안에 있다 (좀비 길을 막지 않는다)', pos.입구보다안쪽);
...
const spot = await pg.evaluate(()=>{
  ...
  /* ★ farmAt() 은 인자를 안 받고 '지금 내 사람이 선 자리'(PL)를 본다.
...
ok('★ 우리 정원이 있다 (무한정 늘려 혼자 다 먹지 못한다)',
   eco.우리정원 > 0 && eco.우리정원 <= 12, '정원 '+eco.우리정원+'마리');   // 29차 — 6 → 10 (선생님)
...
const buy = await pg.evaluate(()=>{
  ...
  o.꽉찬뒤 = W.__farmCount(f); o.정원 = W.__FARM_CAP();
  /* 자원이 없으면 못 산다 */
...
ok('★ 우리 정원을 넘겨 살 수 없다', buy.꽉찬뒤 <= buy.정원, buy.꽉찬뒤+'마리에서 멈춤 (정원 '+buy.정원+')');
...
ok('★ 산물 한 조각이 1칸(사람 한 마리 너비)의 4분의 1을 안 넘는다 — 우리가 산물로 덮이면 동물이 안 보인다',
