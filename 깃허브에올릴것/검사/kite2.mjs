/* (1) 도망치는 사람을 좀비가 따라잡나  (2) 계속 돌면 좀비가 밤을 잊나 */
...
console.log(await pg.evaluate(()=>{
  ...
  /* ★ 수정에서 멀리 떨어진 곳에서 재야 한다.
     예전엔 수정 쪽으로 달리게 했는데, 이제 좀비는 수정이 코앞이면 사람을 놓고 수정으로 간다
     (그게 이번에 넣은 동작이다). 그러면 '못 따라잡았다'가 아니라 '안 쫓았다'가 나온다.
     그래서 수정에서 30칸 떨어진 큰 원을 따라 달리게 한다 — 거의 직선이고 수정은 늘 멀다. */
  /* ★ 나무·바위는 좀비를 막는다(solidTop). 달리는 길에 그게 있으면 '못 따라잡았다'가 아니라
  ...
  L.push('① 좀비 한 마리한테서 똑바로 도망칠 때 (4칸 앞섬 · 최고 속도 · 평평한 빈 들판, 수정에서 멀리)');
  for(const [day,k,nm] of [[1,0,'1일차 좀비'],[8,0,'8일차 좀비'],[14,0,'14일차 좀비'],[8,1,'8일차 날쌘좀비']]){
    G.day=day; G.crystal=G.set.crystalMax; W.__goNight();
    G.wolves.length=0; W.__spawnQ().length=0;
    const w=W.__spawnWolf(k,0);
    /* ★ 평평한 원을 골라 쓴다. 언덕 위로 올라가면 좀비가 닿을 수 없어(높이차 2.2) 쫓기를 그만두는데,
  ...
  /* ② 입구 근처에서 계속 뱅뱅 돌 때 — 좀비가 수정을 잊나 */
  L.push('');
  L.push('② 입구 앞에서 반지름 5칸으로 계속 도는 아이 하나 · 좀비 14마리 · 60초');
