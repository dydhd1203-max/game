     ...
     (34차가 기댄 "콜 하나 60µs" 는 크롬북 계산값이었다 — 25차가 이미 바로잡아 둔 숫자다). */
{
  ...
  const r = await pg.evaluate(()=>{ const W=window, R=W.__R, o={};
    ...
    /* 좀비 조각은 하나도 없으면 visible 이 꺼진다 — 한 번 그려 봐야(flush) 정해진다 */
    W.__drawWolves([], 0, 0.016);
    o.emptyHidden = W.__charMeshes().좀비.every(m => m.count > 0 || m.visible === false);
    return o; });
  ...
  ok('좀비 조각은 하나도 없으면 그리기를 건너뛴다', r.emptyHidden);
   ...
   전체에서 제일 느렸다. 한 페이지에서 낮 항목을 다 재고 -> __setSky(1) 로 밤을 바로 놓고 -> 밤 항목. */
{
  ...
  const r = await pg.evaluate(async ()=>{
    ...
    /* ★ 밝기는 **놀 때 보는 각도**에서 잰다. 위 항목들은 하늘이 보여야 해서 위를 봤는데,
       ...
       이 항목이 걱정하는 것은 '아이가 좀비를 보나' 지 하늘이 아니다 — 재는 자리를 옮긴다. */
    W.__PL.x=16; W.__PL.z=22; W.__PL.yaw=-2.52; W.__PL.pitch=-0.16;
  ...
  ok('★ 하늘 위와 아래의 밝기가 다르다 (색 하나면 같다)', r.위아래차 > 3,
     r.위아래차.toFixed(1));
  /* ★ 밤이 어두워지면 아이가 좀비를 못 본다. 이건 그래픽이 아니라 게임 문제다.
     ★ 띠는 62~88% 다. 처음엔 70~85 로 잡았는데 그건 ACES 시절 이 검사가 83% 를 찍던 때
       ...
       뜻이 있는 경계만 남긴다 — 밤이 낮의 2/3 보다 어두우면 좀비가 안 보이고(62),
       88% 보다 밝으면 밤이 밤 같지 않다. 둘 다 잰 값(70·79)에서 여유가 있다. */
  ok('★ 밤이 낮의 62~88% 밝기다 (어두우면 좀비를 못 보고, 밝으면 밤 같지 않다)',
