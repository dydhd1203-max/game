...
try{
  ...
  const Cr = await page.evaluate(()=>{ const W = window, H = W.__t70, PL = W.__PL, K = W.__KEY, GY = W.__GY, CW = W.__CW, CP = W.__CPLAN, THREE = W.__THREE;
    ...
    /* C1(기록) — 든 총(용의 숨결) 조각 상자 가장 낮은 곳 − 턱 갓돌 윗면(8 + 0.32 + 0.08). 70차 합치기: 가려짐 판정은 아래 C1 픽셀 단계가 한다(층 2 손 패스를 걷고 VM_Z 하나로) */
    ...
    const heldMin = ()=>{ const Hd = W.__held; W.__cam.updateMatrixWorld(true); Hd.updateMatrixWorld(true); box.makeEmpty();
      Hd.traverse(o=>{ if(!o.isMesh || !o.visible) return; let v = o, vis = true; while(v && v !== Hd){ if(!v.visible) vis = false; v = v.parent; } if(!vis) return;
        if(!o.geometry.boundingBox) o.geometry.computeBoundingBox(); bb.copy(o.geometry.boundingBox).applyMatrix4(o.matrixWorld); box.union(bb); }); return {min:box.min.y - GY}; };
    out.C1 = [];
    for(const p of [...galPts.filter((_, i)=> i % 3 === 1), ...strPts.slice(0, 4)]) for(const pitch of [0, -0.3, -0.5]){
      ...
      const m = heldMin(); out.C1.push([p.id, pitch, H.f3(m.min - (8 + CW.CURB + 0.08))]); }
    W.__setAim(false, true); W.__camZoom(3.2);
    /* C7 — 진열 총(상점·대장간 — 손 모형 복제)은 층 0 만(번짐 원천 층 1·옛 손 층 2 0) · 장면 어디에도 층 2 0(70차 합치기 — 손 패스를 걷어 층 2 에만 있으면 안 보인다) · 든 것 조각은 모두 층 0 */
    { W.__drawNPCs(1); const Hd = W.__held, D = W.__DISP(); let dm = 0, d0 = 0, dx = 0, l2 = 0, hm = 0, h0 = 0;
      for(const q of D) q.m.traverse(o=>{ if(!o.isMesh) return; dm++; if(o.layers.mask === 1) d0++; else dx++; });
      W.__scene.traverse(o=>{ if((o.isMesh || o.isLine || o.isPoints || o.isSprite) && o.layers.isEnabled(2)) l2++; });
      Hd.traverse(o=>{ if(!o.isMesh || o.isPoints) return; hm++; if(o.layers.isEnabled(0)) h0++; });
      out.C7 = {disp:D.length, meshes:dm, l0only:d0, other:dx, layer2:l2, held:hm, heldL0:h0}; }
    ...
    return out; });
  put({id:'C7', name:'진열 총 층 — 상점·대장간 진열 총 조각 모두 층 0 만(층 1·2 0) · 장면 어디에도 층 2 0(70차 합치기: 손 패스 걷음) · 든 것(held) 조각은 모두 층 0(본 그림에서 VM_Z 로 그림)',
    pass:Cr.C7.disp >= 10 && Cr.C7.meshes > 0 && Cr.C7.l0only === Cr.C7.meshes && Cr.C7.layer2 === 0 && Cr.C7.held > 0 && Cr.C7.heldL0 === Cr.C7.held, detail:Cr.C7});
  ...
  /* ═══ C1 든 것이 턱·문설·벽에 안 묻힘 — 픽셀(70차 합치기: 성곽 갈래의 층 2 손 패스를 걷고 강화 갈래의 꼭짓점 깊이 누르기 VM_Z 하나로) ═══
     선생님 사진 자리(곧은 성벽 길 안쪽 낮은 턱 앞 · 회랑 안쪽 끝) · 바깥 성가퀴 코앞 · 탑 벽 코앞 · 탑 문설 — 밤 · 용의 숨결 +6 · 1인칭 조준, 그리고 곡괭이(도구).
     한 자리에서 네 장: A 전부 · B 든 것만(세계 숨김) · M 든 것 가림(흰 단색·검은 바탕) · C 세계만(든 것 숨김). M 안에서 B·C 가 다른 픽셀마다
     A 가 B 에 더 가까우면 '보임'. 같은 자리를 누르기 없는 원래 재질(RUNE_BASE·HELD_VC·HELD_VM 원본)로 다시 재서 누르기가 없으면 묻히는 자리가 있음을 보인다(검사가 헛돌지 않게) */
  const C1p = await page.evaluate(()=>{ const W = window, H = W.__t70, PL = W.__PL, GY = W.__GY, G = W.__G, THREE = W.__THREE, R = W.__R, gl = R.getContext(), sc = W.__scene, cam = W.__cam, Hd = W.__held, CP = W.__CPLAN;
    G.phase = 'night'; for(let i=0; i<300; i++){ W.__setSky(1); W.__updSky(0.002, true); } W.__setSky(1); W.__updSky(0, true);
    const wd = gl.drawingBufferWidth, ht = gl.drawingBufferHeight, N = wd*ht*4;
    const A = new Uint8Array(N), B = new Uint8Array(N), M = new Uint8Array(N), C = new Uint8Array(N), white = new THREE.MeshBasicMaterial({color:0xffffff, fog:false});
    const rd = (buf)=> gl.readPixels(0, 0, wd, ht, gl.RGBA, gl.UNSIGNED_BYTE, buf);
    const cc = new THREE.Color();
    const shots = ()=>{ W.__ENH_SPK.pts.visible = false; cam.updateMatrixWorld(true);
      W.__drawFrame(); rd(A);
      const keep = []; for(const o of sc.children){ if(o === cam || o.isLight) continue; keep.push(o, o.visible); o.visible = false; }
      W.__drawFrame(); rd(B);
      const bg = sc.background, fog = sc.fog, ca = R.getClearAlpha(); R.getClearColor(cc);
      sc.background = null; sc.fog = null; R.setClearColor(0x000000, 1); sc.overrideMaterial = white; R.setRenderTarget(null); R.render(sc, cam); rd(M);
      sc.overrideMaterial = null; sc.background = bg; sc.fog = fog; R.setClearColor(cc, ca);
      for(let i=0; i<keep.length; i+=2) keep[i].visible = keep[i+1];
      Hd.visible = false; W.__drawFrame(); rd(C); Hd.visible = true;
      let n = 0, vis = 0, mN = 0;
      for(let i=0; i<N; i+=4){ if(M[i] < 128) continue; mN++;
        if(Math.abs(B[i] - C[i]) + Math.abs(B[i+1] - C[i+1]) + Math.abs(B[i+2] - C[i+2]) < 40) continue; n++;
        if(Math.abs(A[i] - B[i]) + Math.abs(A[i+1] - B[i+1]) + Math.abs(A[i+2] - B[i+2]) <= Math.abs(A[i] - C[i]) + Math.abs(A[i+1] - C[i+1]) + Math.abs(A[i+2] - C[i+2])) vis++; }
      return {mask:mN, n, vis:n ? H.f3(vis/n) : null}; };
    /* 누르기 없는 원래 재질로 잠시 바꿔 잰다(끝나면 되돌림) */
    const RB = W.__RUNE_BASE, rev = new Map(); for(const [b, v] of W.__HELD_VM) rev.set(v, b);
    const raw = ()=>{ const sw = []; Hd.traverse(o=>{ if(!o.isMesh || o.isPoints) return; const m = o.material, b = RB.get(m) || rev.get(m) || (m === W.__HELD_ARM ? W.__HELD_VC : null); if(b){ sw.push(o, m); o.material = b; } });
      const r = shots(); for(let i=0; i<sw.length; i+=2) sw[i].material = sw[i+1]; return r; };
    const WP = W.__WEAPONS, wi = WP.findIndex(x=> x.n === '용의 숨결'); W.__KIT.ownW[wi] = true; W.__equipWeapon(wi); W.__setEnh(wi, 6);
    const pose = (x, z, yaw, pitch, gun)=>{ H.place(x, 8, z, yaw, pitch); W.__camZoom(0); W.__setAim(!!gun, true);
      for(let i=0; i<40; i++) W.__updPlayer(1/60); PL.pitch = pitch; W.__updPlayer(1/60);
      for(let i=0; i<45; i++) W.__updHeld(1/60, false, 0); return [H.f3(PL.x), H.f3(PL.y - GY), H.f3(PL.z)]; };
    const out = [];
    const S2 = CP.straights.find(s=> s.g === 1 && s.s === -1);      // 성문 2(동) 북쪽 곧은 벽 — 선생님 사진 8 자리(51, −20)
    const zS = -20, Tw = CP.towers.find(t=> t.x0 <= 50 && t.x1 >= 52 && t.z1 <= -29 && t.z1 >= -31), D = Tw && Tw.doors.wIn;
    const cases = [
      ['사진8 턱 앞 시선 −0.1', 49.75, zS, Math.PI/2, -0.10, 1, true],
      ['사진8 턱 앞 시선 −0.8', 49.75, zS, Math.PI/2, -0.80, 1, true],
      ['회랑 안쪽 끝 시선 −0.8', null, null, null, -0.80, 1, false],
      ['바깥 성가퀴 코앞', 52.25, zS, -Math.PI/2, 0.0, 1, true],
      ['탑 벽 코앞', null, -29.72, 0, 0.0, 1, true],
      ['탑 문설', null, null, null, 0.0, 1, true],
      ['곡괭이 · 바깥 성가퀴 코앞', 52.25, zS - 1.3, -Math.PI/2, -0.2, 0, true]];
    { const G0 = CP.galleries[0], L = Math.hypot(G0.b.x - G0.a.x, G0.b.z - G0.a.z), cx = (G0.a.x + G0.b.x)/2, cz = (G0.a.z + G0.b.z)/2, e0 = (Math.abs(cx) + Math.abs(cz))*0.6, m = (e0 - 49.74)/(Math.SQRT2*0.6);
      cases[2][1] = cx - G0.N.x*m; cases[2][2] = cz - G0.N.z*m; cases[2][3] = Math.atan2(G0.N.x, G0.N.z); }
    if(D){ const dx0 = Math.max(49.8, Math.min(52.2, D.x1 + 0.6 <= 52.2 ? D.x1 + 0.6 : D.x0 - 0.6)); cases[4][1] = dx0;
      const w = D.x1 - D.x0, cz = (D.z0 + D.z1)/2; cases[5][1] = D.x1 - 0.3; cases[5][2] = cz; cases[5][3] = -Math.PI/2; }
    for(const [nm, x, z, yaw, pitch, gun, withRaw] of cases){ if(x === null) continue;
      const at = pose(x, z, yaw, pitch, gun);
      if(!gun){ W.__setAim(false, true); for(let i=0; i<45; i++) W.__updHeld(1/60, false, 0); }
      const on = shots(), off = withRaw ? raw() : null;
      out.push({nm, at, tool:gun ? 'gun19+6' : W.__curTool(), on, off}); }
    W.__setEnh(wi, 0); W.__setAim(false, true); W.__camZoom(3.2); G.phase = 'day'; for(let i=0; i<60; i++){ W.__setSky(0); W.__updSky(0.01, false); }
    return out; });
  console.log('  C1 자리 · 보임(VM_Z) · 보임(누르기 없는 원래 재질) · 판정 픽셀 — ' + C1p.map(c=> `${c.nm} ${c.on.vis}/${c.off ? c.off.vis : '-'} (${c.on.n})`).join(' | '));
  { const bad = C1p.filter(c=> !(c.on.n >= 150 && c.on.vis >= 0.97)), rawBuried = C1p.filter(c=> c.off && c.off.n >= 150 && c.off.vis < 0.9);
    put({id:'C1', name:'든 것 — 선생님 사진 자리(곧은 성벽 길 턱 앞·회랑 안쪽 끝, 밤, 용의 숨결 +6, 1인칭)·바깥 성가퀴·탑 벽·탑 문설 코앞 · 곡괭이: 든 것 픽셀 보임 ≥ 97%(턱·문설·벽에 안 가려짐 — VM_Z) · 누르기 없는 원래 재질이면 묻히는 자리 ≥ 1(검사가 헛돌지 않음)',
      pass:C1p.length >= 6 && !bad.length && rawBuried.length >= 1,
      detail:{cases:C1p.map(c=> [c.nm, c.at, c.tool, c.on.vis, c.on.n, c.off ? c.off.vis : '-']), bad:bad.map(c=> c.nm), rawBuried:rawBuried.map(c=> c.nm), heldBottom:Cr.C1.map(r=> r[2]).slice(0, 12)}}); }
