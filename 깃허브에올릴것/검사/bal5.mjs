...
const rows = await pg.evaluate(([SHOOT,WPN,MULS,DAYS,FIX,HIT,TOW,ATK,LV])=>{
  ...
  const build=(lv)=>{
    ...
    for(let g=0; g<5; g++){
      ...
      for(let pp=-8; pp<=8; pp+=(TOW==='early'?1.0:0.5)){
      ...
      /* ★ 17차 — 'early' 는 3~7일차에 반이 실제로 지어 놓았을 만한 방어선이다.
         'many'(탑 17채)로 초반을 재면 아무 날이나 다 이겨서 아무것도 안 보인다. */
      const TSET = TOW==='early'
        ? [['arrow',36,-4],['arrow',36,4],['barr',24,0]]
        : TOW==='old'
