...
const who = await ev(()=>{
  ...
  /* 12칸 앞 좀비를 계속 갈겼을 때, 솟은 만큼이 조준 너그러움 안에 드나 */
  const bad = [];
...
ok('★ 카드마다 그 차림새를 입은 사람 그림이 있다 (예전엔 글씨 두 줄이었다)',
...
ok('★ 📘 지능은 캐는 양과 짓기·농장 일을 올린다',
   ...
   '캐는사람 +'+st.지능.amt+' · 짓기 ×'+st.지능.work.toFixed(2)+' · 농장 ×'+st.지능.farm.toFixed(2));
...
/* ★ 이동 속도는 좀비 추격을 깨뜨릴 수 있는 유일한 새 값이라 따로 본다 */
const spd = await ev(()=>{
...
ok('★ 민첩을 다 찍어도 이동 속도가 크게 안 뛴다 (여기를 크게 주면 좀비가 영영 못 쫓는다)',
