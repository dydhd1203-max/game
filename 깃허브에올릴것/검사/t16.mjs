...
const nd = await pg.evaluate(()=>{
  ...
  /* 실제로 캐 봤을 때 들어오는 사람 (표만 고치고 안 쓰면 소용없다) */
  W.__xpReset();
...
/* ★ 17차 — 여기도 20·20·15 를 박아 뒀었다. 자원 사람은 교실에서 겪고 고치는 값이다.
...
const day = await pg.evaluate(()=>{
  ...
  o.무게 = [12,14,15,16,17,18].map(d=> W.__waveFor(d).wt);          // 36차 — 마리수는 구성표 따라 다르니 무게(√체력×무는힘 합)로 본다
  /* 등급도 기준점이 같아야 한다 */
...
ok('★ 15일차 보스가 지옥의 좀비, 18일차가 좀비왕이다',
   day.보스이름[3].includes('지옥') && day.보스이름[4].includes('좀비왕'),
   day.보스이름.join(' / '));
...
ok('★ 마리수가 좀비 렌더 상한(92)을 안 넘는다', day.최대마리 <= 92, day.최대마리+'마리');
ok('무게가 13~14일차보다 늘어난다 (36차 — 마리수 대신 √체력×무는힘 무게)', day.무게[4] > day.무게[1], '마리 '+day.마리수.join(' ')+' · 무게 '+day.무게.join(' '));
